exports.coursewareStudentModuleFactory = ({
    connection,
    CustomEventBridge,
}) => {
    return {
        courseware_studentmodule: async (event) => {
            ...
            if (affectedRows && affectedRows.length > 0) {
                ...
                const ids = affectedRows.map(({ after }) => after.student_id);
                ...
                connection.query(
                    ...
                    function (error, results, fields) {
                        if (error) {
                            console.log(
                                new Date(),
                                "courseware_studentmodule > Erro ao realizar pesquisa de aluno >",
                                error
                            );
                            return;
                        }
                        if (results && results.length > 0) {
                            ...
                            console.log(
                                new Date(),
                                "courseware_studentmodule > Enviando evento para o EventBridge >",
                                eventToSend
                            );
